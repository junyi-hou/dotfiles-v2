    ...
    (with-temp-buffer
      (cl-letf (((symbol-function #'agent-shell--state)
                ...
                ((symbol-function #'agent-shell-prompt-steer)
                 (lambda (&optional _prompt) (plist-put state :steer-called t)))
                ((symbol-function #'agent-shell-prompt-queue)
                 (lambda (_prompt) (plist-put state :queue-called t)))
                ((symbol-function #'gatsby>>agent-shell-read-prompt)
                 (lambda (_prompt-text) "test prompt")))
...
(ert-deftest gatsby>agent-shell-send-or-queue-prompt--prompt-text-distinguishes ()
  "The minibuffer prompt text says whether the prompt will steer or queue,
and the prompt read with it is what the command receives."
  (let ((prompt-texts nil)
        (sent-prompts nil))
    (with-temp-buffer
      (cl-letf (((symbol-function #'agent-shell--state)
                 (lambda () '(:supports-steering t)))
                ((symbol-function #'agent-shell--active-requests-p)
                 (lambda (&rest _) t))
                ((symbol-function #'agent-shell-steering-supported-p) (lambda () t))
                ((symbol-function #'shell-maker-submit) (lambda () (interactive)))
                ((symbol-function #'agent-shell-prompt-steer)
                 (lambda (&optional prompt) (push prompt sent-prompts)))
                ((symbol-function #'agent-shell-prompt-queue)
                 (lambda (prompt) (push prompt sent-prompts)))
                ((symbol-function #'gatsby>>agent-shell-read-prompt)
                 (lambda (prompt-text)
                   (push prompt-text prompt-texts)
                   (format "prompt via %s" prompt-text))))
        (gatsby>agent-shell-send-or-queue-prompt nil)
        (gatsby>agent-shell-send-or-queue-prompt t)
        (should
         (equal
