...
(use-package doom-themes
  ...
  If the prefix arg CURRENT-FRAME is not null, increase fontsize only for the current frame."
  ...
  (gatsby>defcommand gatsby>theme-fontsize-down (current-frame)
    "Decrease the font size in all frames.
  If the prefix arg CURRENT-FRAME is not null, decrease fontsize only for the current frame."
    (interactive)
    (let* ((current-size (plist-get (custom-face-attributes-get 'default nil) :height))
  ...
  (with-eval-after-load 'evil
    ...
    (mapc
     (lambda (hook) (add-hook hook #'gatsby>>set-evil-mode-line-color))
...
(use-package vertico
  ...
  (defun gatsby>>vertico-remove-base (dir)
    ...
    (insert
     (string-trim-right (if (string= dir "~/")
                            (expand-file-name "~/")
                          dir)
                        "[^/]+/?")))
...
current candidate"
    ...
    (cond
     (arg
      (vertico-exit))
     ((directory-name-p (vertico--candidate))
      (vertico-insert))
     (t
      (vertico-exit))))
  ...
  virtually selecting the current candidate, so exit with it."
    ...
    (if (= vertico--total 1)
      ...
      (let* ((common
              (try-completion
               ""
               (if (> (length vertico--candidates) vertico-count)
                   (seq-subseq vertico--candidates 1 vertico-count)
                 vertico--candidates))))

        (cond
         ( ;; virtual selection is done, selecting the candidate
          (memq
           last-command
           '(gatsby>vertico-complete-common-or-select vertico-next vertico-previous))
          (gatsby>>vertico-select-current-candidate-if-not-dir))
         ( ;; has common part
          (> (length common) 0)
