...
(gatsby>use-internal-package eshell
  ...
  (defun gatsby>eshell-setup ()
    ...
    (setq gatsby>eshell-history-cache-file
          (no-littering-expand-var-file-name "eshell-history.el"))
  ...
  (gatsby>defcommand gatsby>eshell-history ()
    ...
    (if (and (featurep 'vterm) (eq major-mode 'vterm-mode))
      ...
      (let* ((eshell-bol
             ...
             (history
              (thread-last
               gatsby>eshell-history-cache-file
  ...
  (defun eshell/tree (&rest args)
    ...
    (cond
     ((executable-find "eza")
      (apply #'gatsby>eshell-alias `("eza" "--tree" ,@args)))
     ((executable-find "tree")
      (apply #'gatsby>eshell-alias `("tree" ,@args)))
     (t
      (user-error "Can't find `exa' or `tree'"))))

  (defun eshell/ls (&rest args)
    (if (executable-find "eza")
        (apply #'gatsby>eshell-alias `("eza" ,@args))
  ...
  (defun gatsby>eshell-cd (cd &rest args)
    ...
    (let* ((host (file-remote-p default-directory))
      ...
      (cond
       ((and (not args)
             root
             (string= (file-truename root) (file-truename default-directory)))
        (funcall cd))
       ((and (not args) root)
        (funcall cd root))
       ((and (not args) host (file-exists-p home))
        (funcall cd home))
       ((and (not args) host)
  ...
  (defun gatsby>eshell-setkey ()
    ...
    (with-eval-after-load 'evil
      (evil-define-key
       ...
       #'eshell/evil-change
       (kbd "C")
       #'eshell/evil-change-line
       (kbd "d")
       #'eshell/evil-delete
       (kbd "D")
       #'eshell/evil-delete-line
       (kbd "<")
      ...
      (evil-define-key 'insert 'eshell-mode-map (kbd "C-r") #'gatsby>eshell-history)
...
(use-package vterm
  ...
  (with-eval-after-load 'evil
    ...
    (evil-define-operator evil-collection-vterm-delete
...
Save in REGISTER or in the kill-ring with YANK-HANDLER."
    ...
    (evil-define-operator evil-collection-vterm-delete-backward-char
    ...
    (evil-define-operator evil-collection-vterm-delete-line
      (beg end type register yank-handler)
    ...
    (evil-define-operator evil-collection-vterm-replace
      (beg end type register yank-handler)
      :motion
      evil-forward-char
      (interactive "<R>")
    ...
    (evil-define-operator evil-collection-vterm-change
      (beg end type register yank-handler)
      (evil-collection-vterm-delete beg end type register yank-handler)
      (evil-collection-vterm-insert))

    (evil-define-operator evil-collection-vterm-change-line
      (beg end type register yank-handler)
      :motion
    ...
    (evil-define-operator evil-collection-vterm-substitute-line
