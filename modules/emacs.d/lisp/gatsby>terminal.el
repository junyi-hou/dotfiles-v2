...
(gatsby>use-internal-pacakge eshell
  ...
  (defun gatsby>eshell-change-buffer-title ()
    ...
    (rename-buffer (format "%s: %s" eshell-buffer-name (directory-file-name
                                                        default-directory)) 'unique))
  (gatsby>defcommand gatsby>eshell-history ()
    "<C-r> in eshell."
    (let* ((eshell-bol (save-excursion
                         (eshell-bol)
                         (point)))
    ...
    (let* ((host (file-remote-p default-directory))
           ...
           (root
            (when-let ((pr (project-current)))
              (project-root pr))))
      (cond ((and (not args) root (string= (file-truename root)
  ...
  (with-eval-after-load 'evil
    ...
    (evil-define-operator eshell/evil-delete (beg end type register yank-handler)
      ...
      (save-restriction
        (narrow-to-region eshell-last-output-end (point-max))
        (evil-delete (if beg (max beg (point-min))
                       (point-min))
                     (if (eq type 'line)
                         (point-max)
                       (min (or end (point-max))
