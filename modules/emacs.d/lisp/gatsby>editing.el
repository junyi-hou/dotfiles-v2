...
(use-package expand-region
  ...
  (defun gatsby>>treesit-expand ()
    ...
    (let* ((root (treesit-buffer-root-node))
           (node (treesit-node-descendant-for-range root (region-beginning)
                                                    (region-end)))
...
(use-package consult
  ...
  to update the correct ring."
    (isearch-update-ring (or gatsby>>consult-current-input
                             (minibuffer-contents-no-properties)) t)
  ...
  (advice-add #'consult-line :around
              #'gatsby>>consult-tempoarily-add-to-minibuffer-hooks)
  (advice-add #'consult-outline :around
              #'gatsby>>consult-tempoarily-add-to-minibuffer-hooks)
  :evil-bind
  ((:maps (motion normal))
